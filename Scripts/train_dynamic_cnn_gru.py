...
OUTPUT_MODEL = "emotion_model_dynamic_cnn_gru.onnx"
SR = 22050
N_MELS = 64
HOP_LENGTH = 512
...
class EmotionCNNGRU(nn.Module):
    def __init__(self, n_mels=64, time_frames=None):
        ...
        self.gru = nn.GRU(
            ...
            num_layers=2,  # 2-layer GRU
            batch_first=False,  # GRU expects [seq_len, batch, features]
            dropout=0.3,
            bidirectional=False  # Can change to True to use bidirectional GRU
        )
...
# Initialize model
model = EmotionCNNGRU(n_mels=N_MELS, time_frames=mel_db_length).to(DEVICE)
...
for epoch in range(EPOCHS):
    ...
    if avg_val_loss < best_val_loss:
        ...
        epochs_no_improve = 0
        torch.save(model.state_dict(), "best_emotion_model_dynamic_cnn_gru.pt")
...
model.load_state_dict(torch.load("best_emotion_model_dynamic_cnn_gru.pt"))
...
plt.tight_layout()
plt.savefig("training_results_dynamic_cnn_gru.png", dpi=150, bbox_inches='tight')
print("Plots saved to training_results_dynamic_cnn_gru.png")
plt.show()
